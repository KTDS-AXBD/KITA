...
> 위임 리서치 산출. **✅ 소스 세트 확정 + 활용신청 완료**(2026-05-25, Playwright 자동):
> - 무역=관세청 data.go.kr **15100475(국가별)·15101609(품목별 총량)** 둘 다 **활용신청 승인** + 이용허락 **"제한 없음"(상업 가능)**.
> - 기업=**OpenDART** 키 검증 완료(status 013=유효, 키 1개가 전 API 커버 — 별도 활용신청 불필요).
> - 뉴스=뉴스빅데이터 메타데이터(fileData 다운로드, P1 후순위).
> - 잔여: 관세청 키 **게이트웨이 전파**(승인 직후 403→200 대기) — 0d 라이브.
