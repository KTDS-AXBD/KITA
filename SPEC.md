...
| F046 | Modal focus trap + a11y 보강 (dialog role + 자동 focus + body scroll lock) 📋 - **갭(F045 차기 후보 답습, 사용자 묶음 지시 7회 연속)**: 공용 `Modal` 컴포넌트가 Esc 닫기·배경 클릭 닫기·닫기 버튼 aria-label만 갖춘 상태로 5가지 a11y 결손. (1) **Focus trap 부재** - Modal 열린 상태에서 Tab/Shift+Tab으로 모달 밖 요소로 focus가 빠져나감 (2) **자동 focus 부재** - Modal 열려도 키보드 사용자는 수동으로 Tab 눌러야 첫 요소 도달 (3) **Focus 복귀 부재** - Modal 닫힐 때 이전 trigger 요소로 focus 안 돌아감(스크린리더·키보드 흐름 단절) (4) **ARIA dialog role 부재** - `role="dialog"`·`aria-modal="true"`·`aria-labelledby` 누락(스크린리더가 모달임을 인식 못 함) (5) **Body scroll lock 부재** - Modal 열린 동안 배경 페이지 스크롤됨. **구현 계획**: ① `useRef<HTMLDivElement>` Modal content + `useRef<HTMLElement | null>` 이전 active element 저장 ② Modal 열릴 때 - body overflow hidden·이전 focus 저장·첫 focusable 요소 자동 focus (`querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])')`) ③ Tab/Shift+Tab 키 핸들러 - 첫/마지막 focusable wrap ④ Modal 닫힐 때 - body overflow 복원·이전 focus 요소로 `.focus()` 복귀 ⑤ `role="dialog"`·`aria-modal="true"`·`aria-labelledby` (title id) 부착·title이 없으면 `aria-label` 직접. **DoD**: typecheck/lint/test PASS + Playwright a11y 실측(Modal 열림 시 첫 focusable 자동 focus·Tab 순회 wrap·Shift+Tab wrap·Esc 닫기 후 trigger focus 복귀·body overflow hidden) + 라이브 배포 @100% + curl /platform/ontology·/platform/cq 302 | KOAMI-REQ-046 | P1 | Master | 📋 |
