...
| F039 | 배포 ✅ — Master 수동: `pnpm install`(cytoscape 동기화)→`deploy:cf`→koami.minu.best. 초기 `1620e4c7`→**리허설 보완(3cb483f) 재배포 `d1a78af9` @100% 활성**(2026-05-27). "Could not find zone" exit1=양성(curl 302·deployments list로 판정). curl `/`·`/platform/data`·`/platform/compare` 302(CF Access 유지). **인증브라우저 7페이지 실렌더 검증 PASS**(2026-05-28, 헤드리스+사용자 교차확인·콘솔에러0·도메인토글 38·51↔44·42·시나리오 5단계+A~E·비교검증 소부장 실값). **버전 선택 랜딩(2026-05-28)**: 루트 `/`=v0.1/v0.2 선택기(`VersionSelectPage`, 셸 없는 스플래시) → **v0.1** 기존 PoC(`/v1` LandingPage+레거시 /scenario/rnd·s6·about·survey, AppHeader 셸)·**v0.2** GIVC Ontology Platform(`/platform/*` 사이드바 셸). 양 셸 브랜드 로고→`/`(선택기 복귀). (직전 '루트→플랫폼 일원화' `c5c40ce3`는 본 선택기로 대체). 재배포 **`f17c560a` @100% 활성**(2026-05-28)·라이브 선택기 렌더 인증브라우저 확인 | KOAMI-REQ-039 | P0 | S22 | ✅ |
