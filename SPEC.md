...
| F042 | v0.2 시연 폴리시 묶음 (PlanPage 타임라인 stale + 시연 동선 next/prev + DataStatusPage sticky·갱신일) 📋 - **갭 3건 발견(/todo plan 2026-05-28)**: (A) `PlanPage` 타임라인 stale - 오늘 5/28인데 Phase 0(5/26~5/30) `done`·Phase 1(6/2~6/6) `active`로 잘못 표시됨(today 기준 status 자동 계산 부재, 하드코딩) → 라이브 화면에 **잘못된 진행 상태** 노출 (B) 7페이지 시연 동선 next/prev 버튼 부재 - 사이드바 순회만 가능, 페이지 하단 다음/이전 버튼 없어 시연 흐름 끊김 (C) `DataStatusPage` UX - 27행 테이블 헤더 비sticky(스크롤 시 컬럼명 사라짐)·KPI 옆 "최종 갱신일" 한 줄 부재·페이지 부제 한 줄만(데이터 카테고리/방법론 설명 누락). **구현 계획**: ① `PlanPage` today 기반 status 계산 함수(`computePhaseStatus(today, startDate, endDate)`) - past=done·current=active·future=upcoming·일정 변경 시 자동 반영 + Phase 4 외부 시연일 미정 표시 ② 신규 공용 `PageNav`(`@/components/platform`, prev/next 페이지 메타 표시·`registry`의 7페이지 순서 기반 자동 산출) + 7페이지 하단 부착 ③ `DataStatusPage` 부제 보강(실/추정/유료 분류 정의·소부장 우선 정렬 안내) + KPI 옆 "최종 갱신일 2026-05-XX" 한 줄(`dataSources.ts` updated 최대값 산출) + `DataTable` sticky 헤더 옵션 추가 + 컴포넌트 stick 적용. **DoD**: typecheck/lint/test PASS + Playwright 7페이지 실렌더(Plan 타임라인 Phase 0=active·Phase 1=upcoming·다음/이전 버튼 동작·Data sticky 헤더 + 갱신일 표시) + 라이브 배포 @100% 활성 + curl /platform/data·/platform/plan 302 | KOAMI-REQ-042 | P1 | Master | 📋 |
