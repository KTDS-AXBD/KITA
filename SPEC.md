...
| F045 | v0.2 잔여 그리드 반응형 audit (Compare/Ontology/CQ/Plan 5개) 📋 - **갭(F044 차기 후보 답습, 사용자 묶음 지시 6회 연속)**: F044에서 ScenarioPage만 처리 후 잔여 5개 그리드 발견. (1) `ComparePage` L129 chatGIVC vs 온톨로지+KG `1fr 1fr` 좌우 비교(**시연 메시지 핵심**) - 모바일에서 stack 필요 (2) `OntologyPage` L46 관계 Modal "출발/도착 엔티티" `1fr 1fr` (3) `OntologyPage` L108 엔티티 정의 13카드 `repeat(3, 1fr)` (4) `CqManagePage` L194 검증 결과 4 메트릭 `1fr 1fr` (5) `PlanPage` L135 Phase 0~4 단계 설명 `repeat(5, 1fr)`(Timeline과 정렬). **구현 계획**: 모두 인라인 `repeat(auto-fit, minmax(N, 1fr))` 변환 - Compare 좌우 큰 카드 minmax(320, 모바일에서 stack), Modal 2칸 minmax(180), 엔티티 13카드 minmax(220), 검증 4 메트릭 minmax(140), Phase 5 설명 minmax(120). **DoD**: typecheck/lint/test PASS + Playwright 1280/700/400 resize 실측(Compare 좌우 wrap·Ontology 카드 wrap·CQ 메트릭 wrap·Plan Phase wrap) + 라이브 배포 @100% + curl 302 | KOAMI-REQ-045 | P1 | Master | 📋 |
