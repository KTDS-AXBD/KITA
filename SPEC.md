...
| F041 | v0.2 정합성 폴리시 (Scenario/Compare/Ontology SSOT 일원화 + 출처메타 일관성) 🔧 - F040 SSOT 파생 패턴을 v0.2 잔여 페이지에 확장. **갭 4건 발견(/todo plan 2026-05-28)**: (a) ComparePage Top3 ↔ ScenarioPage SSOT 드리프트, ComparePage가 표시하는 "대한정밀감속기 94.2점·서원베어링테크 87.6·에스피지 72.3"은 CQ1(호르무즈) PE/PP/합성고무 점수 잔향이고, CQ2 SSOT(`CQ2_TOP5_COMPANIES`) 실값은 0.69/0.51 등이며 에스피지는 CQ2 명단에 없음(미래기계소재가 3위) (b) `CompanyCandidate`·`ImpactItem`·`DependencyItem`·`RnDCase` 인터페이스 모두 `source` 필드 부재 → Badge 부착 불가(`SOBUJIANG_TOP5`/`MINI_IMPACT_NODES`는 있어 내부 불일치, CLAUDE.md "누락 0" 위반) (c) `ScenarioPage` 자체 `CQ_CONFIG[cq2/cq1]` 정의가 cqData SSOT와 별개(F040에서 PlanPage가 폐기한 안티패턴 잔존) (d) `OntologyRelation.source: string`이 분류 없이 혼재(시나리오 정의/GIVC/한국석유공사/산업연관표 등) → BadgeVariant 분류 격상. **DoD**: ① ComparePage Top3을 CQ2 SSOT 파생으로 전환(score×100 정규화·에스피지→미래기계소재) ② 4인터페이스에 source 필드 추가+UI Badge 부착 ③ ScenarioPage CQ_CONFIG 폐기→cqData CQ_SSOT 파생 ④ OntologyRelation.source 구조화(`{ kind: BadgeVariant; label: string }`) ⑤ typecheck/lint/test 87+/build 143모듈 + 로컬 Playwright 실렌더(`/platform/scenario`·`/platform/compare`·`/platform/ontology` 콘솔0·점수 일치) + 라이브 재배포 | KOAMI-REQ-041 | P1 | Master | 🔧 |
