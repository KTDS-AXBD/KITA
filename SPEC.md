...
| F040 | CQ 빌드업 자산화 (질의서 결합 + 산자부 보고 연결) ✅ - v0.2 CQ를 일회성 질문에서 **누적 자산**으로 전환. ① `cqData.ts`(SSOT) 확장: `SURVEY_SCENARIOS`(질의서 S1~S7 매핑, S1 나프타·S3 리튬니켈·S7 한일무역=backlog) + `DATA_INTENT_ITEMS`(질의서 B 데이터의향 10종) + CQ별 `scenarioSid`·`customerAsk`(고객 질의)·`reportRef`(산자부 보고 연결)·`provenance`(기진회 1차 세미나→Phase 질의서→KG 검증→산자부 보고 빌드업 이력) + `dataRequirements.surveyKey`(데이터의향 폐루프). 회신값 `pending`(미회신 placeholder, 폼 미활성). ② `CqManagePage` 상세 패널: 질의서 연결·빌드업 이력(Timeline)·산자부 보고 섹션 + 데이터요구 표 고객회신 열 + 페이지 헤더 시나리오 커버리지. ③ `PlanPage` SSOT 정합(자체 divergent CQ 배열 폐기→`CQ_ITEMS` 파생, CQ-003~007 문구 어긋남 해소). ④ CQ-001 stale Pain Point #6/#7 참조 제거. 검증 typecheck/lint/test **87**/build 143모듈(cytoscape lazy 유지) + 로컬 Playwright `/platform/cq`·`/platform/plan` 실렌더 PASS(콘솔0·커버리지 S1/S3/S7 backlog·S4🥇·S6🥈·빌드업 Timeline 등록→질의서→검증→보고·데이터요구 고객회신 미회신 열·PlanPage SSOT 파생 정합). 커밋 `5e16c19`. **customerAsk 7건 영업 맥락 화법 정제**(기계적 '질의서 A/B 제공 의향' -> demo-script 데이터 유도 톤: 가치 앵커+구체 데이터 요청+우선순위·공유범위 확인, `319a8cb`). **라이브 배포 `297a99e8` @100% 활성**(real 모드 유지·deploy:cf EXIT0 클린·curl `/platform/cq` 302 CF Access) | KOAMI-REQ-040 | P1 | Master | ✅ |
