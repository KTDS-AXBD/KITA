...
| F043 | v0.2 디테일 폴리시 묶음 (ComparePage 출처 대칭 + GraphPage 빈 상태 + PlanPage 푸터 정식 + 반응형 + a11y) 📋 - **갭 5건(F042 차기 후보 답습, /todo plan 사용자 선택)**: (D) `ComparePage` 좌측 LLM 카드는 인라인 텍스트로 출처 "GIVC 산업동향 보고서(2025년 3월호)" 표기하고 우측 KG 카드만 `SourceBadge` 부착 → 비교 축의 정직성 비대칭(시연 핵심 메시지 약화) (E) `GraphPage` NodeDetailPanel이 노드 미선택 시 빈 패널 노출 → "노드 클릭 안내" 빈 상태 카피 부재 (F) `PlanPage` 푸터 "KT DS AX컨설팅팀 \| 2026" → SPEC 정식 "KT DS Enterprise사업본부 AX컨설팅팀" 누락(보고서 신뢰도) (G) KpiCard 4컬럼 그리드(`repeat(4, 1fr)`)·DataStatusPage 등 좁은 폭에서 오버플로우 → `repeat(auto-fit, minmax(N, 1fr))`로 자동 wrap (H) 키보드 포커스·a11y - 모달/사이드바/카드의 focus-visible 확인 + Playwright 콘솔 a11y 경고 점검. **구현 계획**: ① ComparePage 좌측 chatBubble ai 안 "출처: GIVC 산업동향 보고서..." → SourceBadge(virt/추정 - RAG는 1차 데이터 직접 조회 아님) + label 일관 시각 ② NodeDetailPanel 빈 상태 - 도메인별 안내 카피(소부장/호르무즈 도메인 명시) + "그래프에서 노드를 클릭하면 상세가 표시됩니다" + 사용 팁(필터·포커스 토글) ③ PlanPage 푸터 직책 정정 ④ KpiCard 그리드 auto-fit minmax + DataStatusPage 동일 적용 ⑤ a11y - Playwright로 focus-visible 점검, sidebar nav role/aria-label 보강 검증. **DoD**: typecheck/lint/test PASS + Playwright 7페이지 실측(D 좌우 출처 동시 SourceBadge·E 빈 상태 안내 카피·F 푸터 정식·G 좁은 폭 wrap·H 콘솔 경고 0) + 라이브 배포 @100% 활성 + curl /platform/* 302 | KOAMI-REQ-043 | P1 | Master | 📋 |
