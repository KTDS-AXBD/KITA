...
| F044 | ScenarioPage 그리드 반응형 (4·5컬럼 + KPI 일원화) 📋 - **갭(F043 차기 후보 답습, 사용자 묶음 지시 5회 연속)**: ScenarioPage 826줄에 그리드 9개 모두 `repeat(N, 1fr)` 하드코딩 → 좁은 폭에서 오버플로우. L302 의존도 `repeat(5, 1fr)` / L322 KPI `repeat(4, 1fr)` / L383·L484 설명가능성 `1fr 1fr` / L466·L740 3카드 `repeat(3, 1fr)` / L552 선정 프레임워크 STEP1~4 `repeat(4, 1fr)`(외곽 border+overflow:hidden 통합 박스) / L571 핵심 인사이트 `1fr 1fr` / L697 취약성 지표 `repeat(4, 1fr)`. **구현 계획**: ① L322 KPI는 F043 신규 `.op-kpi-grid` 클래스 재사용(일원화) ② 나머지 8개는 인라인 `repeat(auto-fit, minmax(N, 1fr))` 변환 - 짧은 카드(의존도 5·취약성 4) minmax 140px / STEP 4컬럼 minmax 150px / 3카드(반대·대응) minmax 200px / 2컬럼 큰 표(점수 산출·기업 프로필·인사이트) minmax 280px ③ L302·L552 통합 박스는 border+overflow:hidden 유지(wrap 시 모바일에서 2x2 자연스러움). **DoD**: typecheck/lint/test PASS + Playwright resize 실측(데스크탑 1280px 본래 컬럼·700px 자동 wrap·400px 모바일 1~2컬럼) + 라이브 배포 @100% 활성 + curl /platform/scenario 302 | KOAMI-REQ-044 | P1 | Master | 📋 |
